...
import commerce_create_checkout_session_post_4 from "./api/commerce/create-checkout-session/POST";
import config_analytics_get_5 from "./api/config/analytics/GET";
import contact_post_6 from "./api/contact/POST";
...
app.post("/api/commerce/create-checkout-session", commerce_create_checkout_session_post_4);
app.get("/api/config/analytics", config_analytics_get_5);
app.post("/api/contact", contact_post_6);
app.get("/api/health", health_get_7);
// Ensure admin DB tables exist before any admin route runs (lazy, once-only)
app.use("/api/admin", (req, res, next) => {
  runAdminMigrations().then(() => next()).catch(() => next());
});
// Admin public routes (login/logout/session/seed)
